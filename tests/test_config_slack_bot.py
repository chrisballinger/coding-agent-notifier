...
def test_approval_timeout_default_is_300_seconds():
    cfg = parse_config(_ws(enabled=True, bot_token="xoxb"))
    assert cfg.slack.approval_timeout_seconds == 300.0


def test_bot_token_env_resolution(monkeypatch):
    monkeypatch.setenv("MY_SLACK_BOT", "xoxb-from-env")
    cfg = parse_config(_ws(enabled=True, bot_token_env="MY_SLACK_BOT"))
...
def test_env_resolution_missing_var_leaves_none(monkeypatch):
    ...
    cfg = parse_config(_ws(enabled=False, bot_token_env="NO_SUCH_VAR"))
